...
def create_or_update_channel(db: Session, channel: schemas.ChannelCreate):
    db_channel = db.query(models.Channel).filter(models.Channel.name == channel.name).first()
    if db_channel:
        return db_channel
...
def create_or_update_exchange(db: Session, exchange: schemas.ExchangeCreate):
    db_exchange = db.query(models.Exchange).filter(models.Exchange.name == exchange.name).first()
    if db_exchange:
        return db_exchange
